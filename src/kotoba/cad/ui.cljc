...
(defn viewport [props]
  [:section.viewport
   [:div.viewport-tools
    (for [[id label] [[:perspective "Perspective"] [:top "Top"] [:walk "Walk"]]]
      ^{:key id} [button {:class (when (= id (:camera props)) "tool-active")
                          :on-click #((get-in props [:handlers :select-camera]) id)} label])]
   #?(:cljs [viewport/scene-viewport (select-keys props [:scene :camera :time :weather])]
      :clj [fallback-viewport props])
   [:div.scene-chip (str "Kami WGSL / WebGPU · " (str/capitalize (name (:weather props))) " · " (:time props) ":00")]])
